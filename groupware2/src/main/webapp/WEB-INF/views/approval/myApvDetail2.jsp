...
<div class="container">
	...
	<c:when test="${ fn:contains(apvDto.approval_cc, 'Vacat&') || fn:contains(apvDto.approval_cc, 'AM&') || fn:contains(apvDto.approval_cc, 'PM&') }">
		<span>
		<c:forEach items="${fn:split( apvDto.approval_cc , '&') }" var="item" varStatus="stat">
			
    		<c:if test="${ stat.index == 1 }">임시저장 태그 : ${ item }</c:if>
		</c:forEach>
		</span>
	</c:when>
				...
				<div class="modal-header">
					...
					<div class="form-group">
						...
						<div class="modal-footer float-right">
							<button class="btn btn-outline-primary"  type="submit" onclick="funcWantToDel()" >요청</button>
						</div>
