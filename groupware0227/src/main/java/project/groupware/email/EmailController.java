...
public class EmailController {
	...
	public ModelAndView emailWrite(HttpServletRequest req) {
		...
		Email email = emailservice.getAccount(m.getMember_id()+"");
	...
	public String emailWrite(Email email) {
		...
		Email e = emailservice.getAccount(email.getMember_id());
	...
	public ModelAndView emailReceive(HttpServletRequest req) {
		...
		HttpSession session = req.getSession(false);
		Member m = (Member)session.getAttribute("member");
		ArrayList<Email> list = emailservice.getReceive(m.getMember_email());
		Email email = emailservice.getAccount(m.getMember_id()+"");
		mav.addObject("account", email);
	...
	public ModelAndView emailSend(HttpServletRequest req) {
		...
		HttpSession session = req.getSession(false);
		Member m = (Member)session.getAttribute("member");
		ArrayList<Email> list = emailservice.getSend(m.getMember_email());
		Email email = emailservice.getAccount(m.getMember_id()+"");
		mav.addObject("account", email);
		mav.addObject("list", list);
		return mav;
	...
	public ModelAndView emailDetail(@RequestParam(value="email_id") int email_id) {
		...
		Email email = emailservice.getEmail(email_id);
...
//		}		
	...
	public void setEmail(@RequestParam(value="member_id") String member_id) {
		Email email = emailservice.getAccount(member_id);
		if (email != null) {
